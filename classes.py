...
class Session():
    ...
    def start_poll(self, handler):
        print('start poll')
        try:
            self.stop_poll()
        except:
            ...
        self.pollth = Thread(target=self.poll, args=(handler, ))
    ...
    def sendmsg(self,text:str,to):
        if not text.strip() == "":
            print(self.__call__('messages.send',{"text":text,"to_id":to}))
...
def method(method:str,params:dict={}):
    r = requests.post("https://api.wan-group.ru/method/"+method,data=params)
